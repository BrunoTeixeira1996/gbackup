...
func backupGokrPermToExternal(cfg config.Config) error {
	...
	log.Printf("[gokrperm backup info] starting rsync command gokrazy /perm -> external\n")
	for _, t := range sT {
		log.Printf("[gokrperm backup info] performing backup of %s\n", t.Name)
		if err := commands.RsyncCommand(t.Command, "toExternal", cfg.Targets[1].Instance, cfg.Pushgateway.Url); err != nil {
			log.Printf("[gokrperm backup error] could not perform RsyncCommand in %s: %s\n", t.Name, err)
			e = err
		}
	}
	return e
...
func ExecuteGokrPermBackup(cfg config.Config, el *utils.ElapsedTime) error {
	...
	log.Printf("[gokrperm backup info] completed backup of gokrazy /perm to external\n")
